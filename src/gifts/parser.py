...
async def parse_members(client: Client, admin_id: int, chat: str) -> AsyncGenerator[tuple[list[Dict[str, Any]], int, str], None]:
    try:
...
async def get_user_gifts(client: Client, admin_id: int, user_id: int, username: str, result: list | None = None, level: int = 1):
    if result is None:
        result = []
